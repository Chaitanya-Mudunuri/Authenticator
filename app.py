...
def recognize_faces(frame):
    ...
    if result:
        for face in result:
            ...
            cv.rectangle(frame, (bounding_box[0], bounding_box[1]), (bounding_box[0] + bounding_box[2], bounding_box[1] + bounding_box[3]), color, 2)
            cv.putText(frame, f"{name}", (bounding_box[0], bounding_box[1] - 10), cv.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
    return frame
